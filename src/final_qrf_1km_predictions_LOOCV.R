...
## Load the table with coordinates for 25 raster splits 
# This was a necessary step in Kubernetes but note that if the number of the splits done can be decreased (or avoided completely) in other computing environments that would be great, cropping the predictors always takes some time
  ...
  if (c!=1 & c!=21 & c!=25) {
    ...
    for (km in kms) {
      ...
      for (t in 1:length(time)) {
        ...
        NDVI_whittaker_constant_monthly_mean <- rast(paste0("ndvi_", time_alt[t], ".tif")) 
        #plot(NDVI_whittaker_constant_monthly_mean)
        NDVI_whittaker_constant_monthly_mean[NDVI_whittaker_constant_monthly_mean< -200000] <- NA # NAs seen as -248... values
        ...
        NDVI_whittaker_constant_monthly_mean <- terra::crop(NDVI_whittaker_constant_monthly_mean, ext(cropped_exent))
        ...
        water_vegetation_MCD43A4_annual_water_vegetation_sites_low_quality <- water_vegetation_MCD43A4_annual_water_vegetation_sites_low_quality/1000
        water_vegetation_MCD43A4_annual_water_vegetation_sites_low_quality <- terra::crop(water_vegetation_MCD43A4_annual_water_vegetation_sites_low_quality, ext(cropped_exent))
        ...
        water_ground_MCD43A4_annual_water_ground_sites_low_quality <- water_ground_MCD43A4_annual_water_ground_sites_low_quality/1000
        water_ground_MCD43A4_annual_water_ground_sites_low_quality <- terra::crop(water_ground_MCD43A4_annual_water_ground_sites_low_quality, ext(cropped_exent))
        ...
        LST_Day_1km_MOD11A2v006_LST_Day_sites_low_quality <- terra::crop(LST_Day_1km_MOD11A2v006_LST_Day_sites_low_quality, ext(cropped_exent))
        ...
        Percent_Tree_Cover_MOD44B_sites <- terra::crop(Percent_Tree_Cover_MOD44B_sites, ext(cropped_exent))
        ...
        Percent_NonTree_Vegetation_MOD44B_sites <- terra::crop(Percent_NonTree_Vegetation_MOD44B_sites, ext(cropped_exent))
        ...
        Percent_NonVegetated_MOD44B_sites <- terra::crop(Percent_NonVegetated_MOD44B_sites, ext(cropped_exent))
        ...
        for (i in resp_vars) {
          ...
          pred_rast_final<-pred_rast_final[names(modeldata1)[4:37]]
          
          # add modeldata to the prediction data temporarily so that all factor levels are included
          pred_rast_final <- rbind(modeldata1[1:nrow(modeldata1),4:37 ] , pred_rast_final)
