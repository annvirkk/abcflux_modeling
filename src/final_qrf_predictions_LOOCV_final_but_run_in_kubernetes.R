...
for (c in 1:nrow(crops)) { ################ TEMPORARY!!!!! should always be 1:nrow
  # c <- 6
  print(c)
  ...
  for (km in kms) {
    ...
    if (km=="1km") {
      ...
      ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier_ESACCI_CAVM_merged <- raster::crop(ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier_ESACCI_CAVM_merged, ext(crops[c,]))
      ...
      for (i in resp_vars) {
        ...
        for (m in models) {
          ...
          if (km=="1km") {
            ...
            write.csv(pred_matrix, paste0("/mnt/data1/boreal/avirkkala/abcflux_modeling/predictions_1km/", paste(i,  km, m, time[t],c, "loocv", sep="_"), ".csv")) 
