...
### This script is basically identical to the original model prediction script except for a small difference in the model prediction phase
# in this script mod$finalModel and all=TRUE used instead, and 500 predictions (based on the individual trees) are produces and written out
    ...
    for (t in 1:length(time)) {
      ...
      for (i in resp_vars) {
        ...
        for (m in models) {
          ...
          # specify the class for the final model so that we can get the individual tree predictions, see here https://www.ryan-alcantara.com/projects/p89_random_forest_trees/
          ...
          pred <- predict(mod$finalModel, newdata=pred_rast_final, predict.all = T) # need to add mod$$finalModel and predict.all=T to get individual trees
          # the output includes the final mean tree (aggregate) and 500 individual trees
          
          pred <- data.frame(pred$individual)
          ...
          pred <- pred[(nrow(modeldata1)+1):length(pred[,1]),]
          ...
          write.csv(pred_matrix, paste0("/mnt/data1/boreal/avirkkala/abcflux_modeling/predictions_8km_uncertainty/", paste(i,  km, m, time[t],c, "train_loocv", sep="_"), ".csv"), row.names=FALSE) 
          
          print(paste("1 km prediction uncertainty done and written out for ", i, km, m, time[t], c))
