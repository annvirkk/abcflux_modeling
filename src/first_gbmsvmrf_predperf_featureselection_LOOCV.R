...
Baseline_vars_1km_dummy <- c("ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier_ESACCI_CAVM_merged80",
                             ...
                             "ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier_ESACCI_CAVM_merged21",
                             "ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier_ESACCI_CAVM_merged33",
                             "Number_of_days_since_fire_classes_MCD64A1_sites_cleaned1",
                             "Number_of_days_since_fire_classes_MCD64A1_sites_cleaned3",
                             "TKHP_Thermokarst3",
                             "TKHP_Thermokarst1",
                             "TKWP_Thermokarst1",
                             "TKWP_Thermokarst2",
...
for (i in resp_vars) {
  ...
  modeldata2 <- d[,c("Study_ID_Short", "id", i, Baseline_vars_1km, Baseline_vars_1km_dummy)]
  modeldata1 <- na.omit(modeldata2) # only 86 obs
  sapply(modeldata1, function(x) sum(is.na(x))) # no missing data
  print("1 km data set:")
  print(nrow(modeldata1))
  
  modeldata2 <- d[,c("Study_ID_Short", "id", i, Baseline_vars_20km, Baseline_vars_20km_dummy)]
  ...
  modeldata11 <- merge(modeldata1[ , !(names(modeldata1) %in%  c("Study_ID_Short",  i, Baseline_vars_1km, Baseline_vars_1km_dummy))], d, by="id")
  modeldata22 <- merge(modeldata2[ , !(names(modeldata2) %in%  c("Study_ID_Short",  i, Baseline_vars_20km, Baseline_vars_20km_dummy))], d, by="id")
  ...
  for (km in kms) {
    ...
    for (m in models) {
      ...
      # Load model files
      mod <- readRDS(paste0("/mnt/data1/boreal/avirkkala/repos/abcflux_modeling/results/", paste(i,  km, m, "loocv", sep="_"), ".rds")) # jostain syystä sanoo 10-fold cv???
      ...
      preds_gbm <- gbmFit$pred %>%
        filter(Variables==mod$bestSubset)
      
      preds_rf <- rfFit$pred %>%
        filter(Variables==mod$bestSubset)
      
      preds_svm <- svmFit$pred %>%
        filter(Variables==mod$bestSubset)
      ...
      # names(pred_ens)
      # ggplot(pred_ens_all, aes(x=pred.x, y=obs)) +
      #   geom_point(shape = 16,  size=3) + geom_abline(slope = 1) + 
      # 
      #   xlim(scale_min, scale_max) + ylim(scale_min, scale_max) 
      
      mae <- mae(pred_ens_all$obs, pred_ens_all$pred_ens)
      ...
      p10 <- ggplot(pred_ens_all, aes(x=pred_ens, y=obs, colour=factor(Disturbance))) +
        ...
        labs(color='Disturbance history\nof the site') +
      ...
      if (m=="rf") {library(randomForest)} # https://stackoverflow.com/questions/38623624/usemethodpredict-no-applicable-method-for-predict-applied-to-an-object-o
      if (m=="svm") {library(kernlab)} # https://stackoverflow.com/questions/38623624/usemethodpredict-no-applicable-method-for-predict-applied-to-an-object-o
      ...
      if (m=="svm") {
        ...
        varImp <- vip::vi(mod, method="permute", train=obspred[, Selected_vars], target = obspred[, i], metric = "rmse",
      ...
      #   varImp2 <- varImp
      #   
      #   if (startsWith(varImp2$Variable, "ESACCI_cavm_general_ESAwaterfix_broadevfix_mixfix_cropfix_nowaterglacier")) {
      #     
      #     # gather all columns that start with this and sum
      #     
      #     
      #   }
      #   
      ...
      # TESTING WHAT*S GOING ON WITH SVM
      # missing <- Baseline_vars_1km[!(Baseline_vars_1km %in% Selected_vars)]
      # varImp <- vip::vi(mod, method="permute", train=obspred, target = obspred[, i], metric = "rmse",
      #                   pred_wrapper = predict, nsim=100)
    ...
    all_varImp <- merge(all_varImp, test, by="Variable")
