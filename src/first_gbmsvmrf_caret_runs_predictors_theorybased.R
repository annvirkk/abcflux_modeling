...
# install.packages("mboost", lib="/mnt/data1/boreal/avirkkala/packages")
# install.packages("plyr", lib="/mnt/data1/boreal/avirkkala/packages")
# install.packages("import", lib="/mnt/data1/boreal/avirkkala/packages")


library("mboost", lib.loc="/mnt/data1/boreal/avirkkala/packages")
library("plyr", lib.loc="/mnt/data1/boreal/avirkkala/packages")
library("import", lib.loc="/mnt/data1/boreal/avirkkala/packages")
...
Baseline_vars_1km <- c("srad_terraclimate_sites",  "pr_terraclimate_sites", "pdsi_terraclimate_sites",  #"tmean_terraclimate_sites",  dropped because LST seemed to be more important
                       # "vpd_terraclimate_sites", # dropped this because of multicollinearities
                       
                       "terra_trend_19812010",# temperature change - note that the naming convention changed a bit...
...
for (flux in resp_vars) {
  ...
  ### IN THEORY-BASED SCRIPT, THIS CAN BE CLEANED UP TO FOCUS ON TRAIN ONLY
  ...
  # for some reason using the same way to code the train function as above doesn't work with factor levels, but specifying it in a different way works:
  rfe_fit = train(as.formula(paste(flux, "~", paste(Baseline_vars_1km, collapse="+"))),
                  trControl = traincontrol1, # rfe parameters
                  method="svmRadial", data=modeldata1)
  # I also tested that the performance estimates from this are similar to the one that you get from running the same model with factors coded as dummy variables
  ...
  set.seed(448)
  # for some reason using the same way to code the train function as above doesn't work with factor levels, but specifying it in a different way works:
  rfe_fit2 = train(as.formula(paste(flux, "~", paste(Baseline_vars_20km, collapse="+"))),
                  trControl = traincontrol2, # rfe parameters
  ...
  ### Write the model out
  saveRDS(rfe_fit, paste0("/mnt/data1/boreal/avirkkala/repos/abcflux_modeling/results/", paste(flux, "predictors_theory", "1km_gam",  sep="_"), ".rds"))
  ...
  ### Write the model out
  saveRDS(rfe_fit2, paste0("/mnt/data1/boreal/avirkkala/repos/abcflux_modeling/results/", paste(flux, "predictors_theory", "20km_gam", sep="_"), ".rds"))
