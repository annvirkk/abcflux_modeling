...
for (i in resp_vars) {
  ...
  for (m in models) {
    ...
    print(p10)
    dev.copy(png, paste(i, m, km, "predperf_residuals.png", sep="_"), width=900, height=750)
    dev.off()
    ...
    varImp <- vi(mod, method="permute", train=obspred[, Selected_vars], target = obspred[, flux], metric = "rmse",
                   pred_wrapper = predict, nsim=100, verbose=TRUE) # only 10 most important ones selected - ask package developer if we need more
    
    varImp <- vi(mod, method="permute", train=obspred, target=flux, metric="rmse", pred_wrapper=predict) 
